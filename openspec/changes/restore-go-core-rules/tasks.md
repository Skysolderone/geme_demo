...
- [ ] 6.2 新规则基线（**负责人 2026-09-22 裁决：本 change 只跑 20 局冒烟**——AI 尚无停手阈值，200 局几乎必然接近 100% 截断、数据无意义；完整 200 局移到 `life-shape` 4.4 与 `ai-eye` 校准）：4 人 v5 图、种子 1–20、同强度 AI，出报告；重点记录截断率、终局原因分布、平均结束大回合、单串军势峰值、倍增子选择率、第 3 大回合领先者胜率。验证：报告放 `sim-out/`，关键数字写入 implement 记录，明确标注"AI 权重未校准口径"。
