...
- [x] 2.1 先写测试：「3 人基准地图」三个 Scenario。验证：先红。
- [x] 2.2 编写 `maps/siege-3p-base-v1.json` 并登记内置表、选图界面。验证：静态校验与预算通过；2.1 全绿；变异（破坏镜像应红；把某区到中央入口距离拉开 2 时"三区公平"应红）。
- [x] 2.3 Godot 在 3 人图上三条自检退出码 0；截图与文本盘面同 1.4（`3p-*`）。
- [x] 2.4 3 人图标准难度 20 局冒烟，同 1.5（`3p-smoke20/`）。
- [x] 2.5 设计文档 §3.2 / §3.3 补两张图说明，版本 v1.12 → v1.13，变更记录加一行；冒烟数字附上并注明"AI 未在小图上校准，区间是否重估另议"。
- [x] 2.6 全量回归：两处 `dotnet build` 零警告；`dotnet test -c Release` 全绿；`openspec validate small-maps --strict` 通过（不跑 200 局规模慢测试）。
