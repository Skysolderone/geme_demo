...
public class 信息层的进入与退出Tests
{
    ...
    public void 键鼠与手柄分别配置()
    {
        ...
        bindings.Rebind(InputDevice.KeyboardMouse, TacticalLayer.Board, "key:T");
        Assert.Equal("key:T", bindings.BindingOf(InputDevice.KeyboardMouse, TacticalLayer.Board));
        Assert.Equal(gamepadBefore, bindings.Snapshot(InputDevice.Gamepad));
        Assert.Equal(TacticalLayer.Board, bindings.Resolve(InputDevice.KeyboardMouse, "key:T"));
        Assert.Null(bindings.Resolve(InputDevice.Gamepad, "key:T"));

        var keyboardBefore = bindings.Snapshot(InputDevice.KeyboardMouse);
        bindings.Rebind(InputDevice.Gamepad, TacticalLayer.Board, "pad:y");
        Assert.Equal(keyboardBefore, bindings.Snapshot(InputDevice.KeyboardMouse));
        Assert.Equal(TacticalLayer.Board, bindings.Resolve(InputDevice.Gamepad, "pad:y"));
        Assert.Null(bindings.Resolve(InputDevice.Gamepad, "pad:dpad_up"));

        // 同一设备内一个键只控一层：把气层也绑到 key:T，领地层失去该键
        bindings.Rebind(InputDevice.KeyboardMouse, TacticalLayer.Power, "key:T");
        Assert.Equal(TacticalLayer.Power, bindings.Resolve(InputDevice.KeyboardMouse, "key:T"));
        Assert.Null(bindings.BindingOf(InputDevice.KeyboardMouse, TacticalLayer.Board));
        Assert.Equal("pad:y", bindings.BindingOf(InputDevice.Gamepad, TacticalLayer.Board));
    }
    ...
    public void 辅助设置切换为点击()
    {
        ...
        foreach (TacticalLayer layer in new[] { TacticalLayer.Power, TacticalLayer.Power, TacticalLayer.Relics, TacticalLayer.Relics, TacticalLayer.Board })
        {
        ...
        Assert.Equal(TacticalLayer.Board, viaToggle.Active);
