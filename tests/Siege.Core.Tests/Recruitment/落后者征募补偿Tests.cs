...
/// 阶梯盘面（9×9 合成图，四角各一簇）势力可独立复算：势力 = 棋子基础军势 + 独占空格数（四邻接）。
/// P0 A1-D1 四枚 → 4 + 5（A2 B2 C2 D2 E1）= 9；P1 G1 H1 J1 三枚 → 3 + 4（G2 H2 J2 F1）= 7；
/// P2 A9 B9 两枚 → 2 + 3（A8 B8 C9）= 5；P3 J9 一枚 → 1 + 2（J8 H9）= 3。名次 1 / 2 / 3 / 4。</para>
...
public class 落后者征募补偿Tests
{
    ...
    {
        // 规格 Scenario「并列最后一名都获得补偿」：势力 9 / 7 / 5 / 5 → C、D 共享名次 3（竞争名次，最大名次是 3 不是 4），二者均展示 +1、选取 +1。
        // P3 补一枚 H9 与 P2 的 A9 B9 镜像对称：2 + 3（J8 G9 H8）= 5。
    ...
    public void 与信物加成相加()
    {
        ...
        // D 的唯一一枚棋子就落在信物格 J9 上（占据即控制），故势力仍为 3、名次仍为 4。
