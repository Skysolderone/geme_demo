...
/// <summary>
/// frontier-map tasks 3.4：<c>map</c> 子命令能按地图标识打印边疆图的文本图（高度 / 地表 / 桥 / 栅栏 / 平台编号 / 据点 / 信物）与校验报告项
...
public class 地图子命令Tests
{
    ...
    public void 按标识打印边疆图的文本图与距离报告项()
    {
        ...
        (int code, string text, string err) = RunMain("map", "--map", "siege-frontier-v1");
    ...
    public void 不带选项仍打印缺省地图且未登记的拼写被拒绝()
    {
        ...
        Assert.Equal(0, code);
        Assert.Contains("地图 siege-4p-base-v4  13×13", text, StringComparison.Ordinal);

        (int badCode, _, string err) = RunMain("map", "--mapp", "siege-frontier-v1");
