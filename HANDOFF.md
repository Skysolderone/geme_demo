...
> 更新于 2026-09-16（首轮原型 8/8 完成，图形界面可玩；打磨轮进行中：地图加密已归档，AI 安全权重校准进行中）。仓库：`git@github.com:Skysolderone/geme_demo.git`。
...
## 数据现状（200 局回归，denser-map 之后，种子 1–200，Standard，`sim-out/denser-map-200/`）

| 指标 | 实测 | §16 目标 |
|---|---|---|
| 部署上限中位数 第 1–3 / 4–6 / 7+ | 3 / 4 / 5 | 3 / 3–5 / 5–8 |
| 首次跨出生区冲突 | 第 5.46 大回合 | 4–5（超 0.46） |
| 首次冲突时盘面占用率 | 65.3% | — |
| 终局局平均结束大回合 | 9.43 | 7–10 |
| 终局原因 整轮Pass / 达上限 / 碾压 | 67.5% / 27.5% / 5.0% | — |
| 第 3 大回合领先者胜率 | 64.5% | ≤ 50%（超 14.5） |
| 整局从未提子的局 | 60 / 200 | — |

**这组数据有一个已定位的失真源**：`EvaluationWeights.Default.Safety = 20` 是 v1 空旷地图上凭 5 局试出的初值，
它自己的注释就标着"阶段 B 的首个校准项"，校准从未做过。v2 上九档各 200 局扫档（`sim-out/safety<N>/`）：

| Safety | 3 | **5** | 7 | 8 | 10 | 20（旧值） | 30 | 40 | 60 |
|---|---|---|---|---|---|---|---|---|---|
| 不收敛率 | 10.0% | **9.5%** | 3.5% | 13.0% | 31.5% | 27.5% | 34.0% | 42.0% | 57.5% |
| 首次冲突 | 4.05 | **4.68** | 4.95 | 5.08 | — | 5.46 | — | — | — |
| 整局无冲突 | 0 | **3** | 10 | 6 | — | 60 | — | — | — |
| 领先者胜率 | 16.0% | **51.0%** | 61.5% | 60.5% | — | 64.5% | — | — | — |

曲线非单调且调高更糟：安全权重越高，被威胁的棋串越是总有"补一口气就加分"的手，AI 因此永不 Pass。
取 5 不取不收敛率最低的 7，因为 7 的领先者胜率 61.5% 与旧值没差别；取 5 不取 3，因为 3 的领先者胜率
跌到 16.0%，低于 25% 的随机基线（早期领先反而变劣势）。裁决见 `openspec/changes/ai-safety-weight/design.md`。

**另一个未解问题**：按名次分组统计（第 5 大回合起，120 局），四家部署上限都是约 5，实际落子
1.61 / 0.92 / 0.68 / 0.75，Pass 率 22.9% / 42.7% / 52.8% / 45.2%。**落后者不缺棋子额度，缺的是值得落的点**——
这解释了为什么征募端的补偿在密地图上失效。需要独立一轮，补偿要给空间而不是牌。
...
- `maps/siege-4p-base-v1.json` 已无法加载（109 格超出新区间 80–95），只能人工比对，不能开局。
...
- 4 人基准地图：`siege-4p-base-v2`（内置于 `FourPlayerBaseMap`，导出物 `maps/siege-4p-base-v2.json`）——D2 对称、外接 11×11、可落子 85、障碍 36（29.8%）、四区各 13 格、14 信物格、四区到公共信物/中央入口/咽喉距离 2/6/3 极差 0
