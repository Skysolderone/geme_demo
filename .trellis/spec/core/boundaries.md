...
| 活形 / 禁入（空区、封闭眼空间、眼值、三态、禁入格） | `LifeShapeReport`（`Siege.Core.Board`，life-shape）：`Analyze(GameBoard)` 全量重算、不缓存、不入存档；空区只经 `GameBoard.LibertyNeighbors` 取邻接（源码扫描守门），`EyeSpaceMax = 12` 全仓只此一个字面量。预演（第 1 步 `IsForbiddenFor`、第 6 步按棋子重查 `GroupLifeAt`）、合法落子范围契约、公开视图（`MatchPublicView.LifeShape`，`Publish` 是唯一构造点）、表现层、终端、AI（候选来自已扣除禁入格的范围）与遥测（`MatchSession.LifeEntry`）**共用这一份查询**，不得各写一份眼 / 禁入判定。**从格集合里扣除禁入格只在 `MatchFlow.LegalRangeFor` 一处**（裁决 R12）；读取 `ForbiddenCellsFor` / `IsForbiddenFor` / `ProtectedCellsOf` 做标示与统计是放开的。守门 `合法落子范围的对外契约Tests.禁入扣除只在LegalRangeFor且与预演共用同一查询`（扫 Core / Sim / Presentation / `src/godot` 的 `Except…`、`Where(!…IsForbiddenFor)`、`Remove…` 三种扣除形态 + 逐格预演与契约扣除的行为比对，变异 M-D1 / M-D2 已证红）；表现层与 Godot 不调 `Analyze`（`表现层不调用活形分析只读公开视图`、`Godot层不调用规则计算入口`）。`GroupSafety.EyePoints`（AI 启发式）暂与之并存、共用气边查询，去留归 `ai-eye` |
