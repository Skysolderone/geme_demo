# 09-12-heuristic-ai — 技术设计

## 权威来源

本任务的架构决策、接口契约、风险取舍与已确认裁决，全部以 **`openspec/changes/add-heuristic-ai/design.md`** 为准。实现前 MUST 完整阅读该文件，重点是：
...
**代码位置**：`src/Siege.Core/Ai + Siege.Sim` —— 启发式评价与候选剪枝；批量跑局、日志与平衡报告
...
- 本任务尚无线上形态，回滚即回退提交。
- 若本任务修改了已被下游任务依赖的契约，MUST 同步更新 `openspec/changes/add-heuristic-ai/design.md` 的接口契约表，并在受影响的子任务 `prd.md` 中记录。
