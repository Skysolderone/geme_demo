...
负责人试玩后要求：平台（出生区）内部的障碍物全部移除、留白；每个平台保留 1–2 个信物格与 1 个营帐（design 裁决 17）。基线 1244 项 → 段末 **1245 项全绿**（+1），`dotnet build -c Release --no-incremental` 0 警告，`dotnet build src/godot/Siege.Godot.csproj` 0 警告，`openspec validate frontier-map --strict` / `map-generator --strict` 通过。未提交。

### 改动文件

| 文件 | 内容 |
|---|---|
| `src/Siege.Core/Board/Maps/FrontierMapV1.cs` | 字符画六个平台方块里的 34 个 `#` 改成平台格（1 号 11：F28 G28 C27 C26 H26 G25 H25 J24 B23 C23 E21；2 号 10：T8 X8 T7 Y7 Y6 V5 W5 U3 W3 R2；3 号 6：X28 T26 U26 T23 U23 W22；4 号 5：D8 H6 E5 G3 J3；5 号 1：F15；6 号 1：T16）；解析器收紧——方块内出现 `#` 当场抛出；注释同步。标识不变。过渡带 / 填充区一格未动 |
| `maps/siege-frontier-v1.json` | `map --map siege-frontier-v1` 重新导出（与代码逐字节一致，由「生成器确定且与磁盘文件一致」「内置图不受生成器影响」兜底）。**旧日志 / 旧存档回放会因内容摘要不同报"地图不一致"，属预期** |
| `FrontierMapLayout.Fill.cs` | 删 `ScatterPlatformRocks` / `ScatterOn` / `PlatformStaysWhole`；留下的 `TopUpOpenGround` 只做"超上限即作废 + 偏少补空地" |
| `FrontierMapLayout.cs` / `FrontierMapGenerator.cs` | 删 `Cell.PlatformRock` 及其灌数据分支；`CheckLayout` 的 80% 自检改为"平台可落子格 ≠ 外接面积即拒绝" |
| `FrontierMapLayout.Platforms.cs` | 外接面积上限 `320 − 5N`（295 / 290 / 285 / 280）→ 按平台数 **295 / 270 / 255 / 240** |
| `FrontierMapLayout.Corridors.cs` | 平台数 ≥ 7 时走廊一律 2 格宽（随机数照抽，消费次序不变） |
| `FrontierMapLayout.Checks.cs`、`src/Siege.Sim/Running/MatchSession.cs`、`src/Siege.Core/Ai/AiDifficulty.cs` | 只改过时注释 |
| 规格 / 设计 | `frontier-map/specs/map-definition`（bullet、Scenario「平台规模」、新 Scenario「平台留白」）；`map-generator/specs/map-generation`（布局规则第一条、Scenario「平台规整」）；`map-generator/design.md`（D1 第 1 / 3 / 5 步、裁决 17）；两份 `tasks.md` 里的 80% / 20% 字样；`frontier-map` 的 ai-decision / viewport-camera 规格与 `.trellis/spec/core/testing.md` 里的"377 格" → 411 |

### 手工图新统计

可落子 **377 → 411**（边疆档 300–420 内，**无需削减过渡带**）；h0/h1/h2 = 105/26/280；各平台 70/54/43/31/24/24 → **81/64/49/36/25/25**；岩石 221 → 187。信物 16（平台内 9）、据点 16、四座桥、四段栅栏、各平台到五类目标的距离（中央入口 10/10/11/11/4/4 等）全部不变；必死口袋豁免仍为空；保护期容量、可达性、"小平台更靠中央"等既有测试未改、全绿。

### 既有测试更新（逐条，均因需求变更）

1. `边疆档基准地图Tests.平台规模`：`zone.Count*100 >= side²*80` → `zone.Count == side²`；"外接方块里不属于平台的格只能是岩石"那条在新图上成了空集上的恒真断言，删去，由新测试「平台留白_平台内无障碍格」接替；注释里 M-B1 的描述改为"构造当场抛出"。
2. `生成图布局规则Tests.平台规整`：`Count*5 >= side²*4` → `Count == side²`；"方块内不属于出生区的格只能是岩石" → "方块（取生成器报告的外接方块，不从出生区反推）内无障碍格、每格都是本平台可落子格"。
3. `生成确定性Tests.生成图的黄金值_gen12345`：尝试序号 8 → **0**、可落子 334 → **370**、SHA-256 `D8F510CA…FEA203` → **`5DABB9BF…4FCF3986`**。原因：平台数 6 的面积上限变了（边长重采序列变）+ 去掉撒石后随机序列前移，全部生成图重排。
4. `地图子命令Tests.按标识打印边疆图的文本图与距离报告项`：`各 70/54/43/31/24/24` → `各 81/64/49/36/25/25`；第 15 行手写期望里 F15 的 `##` → `25`。

新增：`边疆档基准地图Tests.平台留白_平台内无障碍格`（方块内无障碍、每格是本平台可落子格、每平台恰 1 营帐与 1–2 信物、各平台格数、可落子 411）。生成图一侧的"平台内无障碍"断言并在「平台规整」里（种子 1–50 × 平台数 5–8）。其余既有测试未改期望。

### 变异验证（脚本二进制读写、finally 还原；每条跑 5 个相关测试类 79 例；末尾全量复绿 1245）

| 编号 | 变异 | 结果 |
|---|---|---|
| M-B1r | 字符画 F15 改回 `#` | 红（失败 20 例 / 14 个测试名）：构造当场抛出，`边疆档基准地图Tests` 全类 + `内置图不受生成器影响` + `地图子命令` |
| M-B15 | 解析器恢复"方块内允许 `#`" + F15 改回 `#` | 红 5：`平台留白_平台内无障碍格`、`平台规模`、磁盘一致 ×2、`地图子命令` |
| MG-18 | 补空地前往 1 号平台内放一格岩石（自检保留） | 红（失败 54 例 / 20 个测试名）：`CheckLayout` 逐次拒绝 → 耗尽 64 次，生成类测试全红 |
| MG-19 | 同上 + `CheckLayout` 放宽回 80% | 红：`平台规整`（4 个平台数）+ gen:12345 黄金值——证明新断言本身生效 |
| MG-19b | 只把 `CheckLayout` 放宽回 80% | 绿——构造已不产生平台内障碍，自检是重复防线，非测试缺口 |
| MG-20 | 平台数 6 的面积上限 270 → 290 | 红 1：gen:12345 黄金值 |
| MG-21 | 去掉"平台数 ≥ 7 一律 2 格宽" | 绿——这是调分布的杠杆，规格不要求（"主干 2–3 格"两种都满足），黄金值只钉平台数 6 的一张图 |

### 生成器种子 1–50 新统计（Release，本机；`sim-out/mapgen-stats.txt`）

| 平台数 | 尝试序号分布 | 可落子格 | 中位数 / 最大耗时 |
|---|---|---|---|
| 5 | 0→13，1→11，2→4，3→4，4→6，5→2，6→2，7→2，8→1，9→2，12→1，13→1，14→1 | 316–396 | 22 ms / 230 ms（首张含 JIT） |
| 6 | 0→16，1→15，2→10，3→3，4→1，5→1，6→2，8→1，10→1 | 317–410 | 12 ms / 24 ms |
| 7 | 0→17，1→9，2→8，3→7，4→5，5→2，6→1，20→1 | 333–411 | 21 ms / 62 ms |
| 8 | 0→11，1→8，2→10，3→7，5→1，6→2，7→2，8→1，9→2，10→1，11→1，12→1，14→1，15→1，17→1 | 351–408 | 24 ms / 46 ms |

200 张全部通过校验、无一耗尽 64 次（最坏 20）；走廊宽度 ≥ 2 / 无一子堵死格 / 桥长 ≤ 3 的布局规则测试全绿。`gen:12345`：尝试序号 0、可落子 370、平台 8/8/6/6/5/5、桥 2+3。速览三份文件已重出。

**预算怎么定的（临时探针，已删）**：只删撒石、其余不动时，种子 1–50 的可落子升到 316–420，"超出 420 作废"在平台数 6 / 7 / 8 各触发 4 / 3 / 13 次（平台数 8 最坏尝试 20）。数据说明**面积上限不是主因**：超限样本的平台面积 211–272，全在原上限之内；方差来自过渡带格数（同一平台数下 84–222，摆幅约 90，而平台面积的摆幅只有约 50）——平台数 8 把面积上限压到 215（只剩 2 种边长组合）仍有超限。试过的杠杆：冗余边按运行计数跳过（几乎无效，冗余边多是并线）、走廊一律 2 宽（平台数 8 超限 13 → 5）、收面积上限。定稿 = 面积上限 295 / 270 / 255 / 240 + 平台数 ≥ 7 一律 2 宽。种子 1–50 ∪ 1001–1100（每档 150 张）上"超限作废"的触发率：平台数 5 / 6 / 7 / 8 = 0/640、5/422、3/472、8/683 次尝试（约 1%），平均尝试序号 3.3 / 1.8 / 2.2 / 3.6，最坏 20；边长组合种数 41 / 47 / 21 / 6。补空地（目标 340–380）未动——它只往上补，与超限无关。

### Godot 自检与截图

`--auto-demo --pick-check`：`--map=siege-frontier-v1`（逐格居中 411/411 ×2）、`--map=gen:12345`（370/370 ×2，中心位姿遮挡 4 格、失败 0）、`--map=gen:987654321:p8`（367/367 ×2，遮挡 3、失败 0）、不带 `--map` 的 v4（105/105 ×2）——四条退出码均为 0。`BoardView` 平台描边里"方块内不可落子格算洞"的分支保留、不再触发。

截图（未读入上下文）：`sim-out/mapgen-shots/frontier-clean-overview.png`、`sim-out/mapgen-shots/gen-12345-clean-overview.png`。

### 偏离与待裁决

1. **"超出边疆档上限即作废"的安全网仍会触发（约 1% 的尝试）**。任务要求"优先收紧面积预算、不靠重试碰运气"；实测面积预算不是主因（见上），已按平台数收紧 + 平台数 ≥ 7 走廊 2 宽，把触发率从平台数 8 的约 2% 压到约 1%。要归零需要一个往下调的手段（例如超限时把开阔处的过渡带格回填成岩石，须同时守住"2×2 宽度"与"一子堵死"两条口径），或动摆位（带间缝）——超出本次修订范围，未做，请裁决是否需要。尝试上限 64 未动。
2. **边长组合变少**：平台数 6 的面积上限 290 → 270，手工图那样的 9/8/7/6/5/5（面积 280）生成器不再抽得到；平台数 8 只剩 6 种组合（以 5、6 为主，偶有一个 7）。
3. **营帐总落在平台的角上**：营帐取"离缓坡最远的格"，平台留白后这必然是某个角（改前多数也是，现在是全部）。要不要改成"离缓坡远、但不贴边"待定，本次未动。
4. `AiDifficulty` 的大图候选格上限是在 377 格的旧图上实测选定的；411 格后未重测（`候选格上限Tests` 全绿）。
5. `siege-frontier-v1` 标识未改、内容已变：仓库里没有入库的回放日志 / 存档夹具依赖旧内容（全量测试无此类失败）；`sim-out/` 下的旧日志回放会报"地图不一致"。

## 修订：平台内不放营帐（2026-09-21）

负责人追加裁决（design 裁决 18，B 方案）：平台上的营帐也去掉，平台上只留 1–2 个信物格，得分点全部在平台外。在「修订：平台留白」的未提交工作树上继续；上一节里"每平台恰 1 个营帐""营帐总落在平台角上（待裁决 3）"等表述以本节为准（后者已不存在）。基线 1245 → 段末 **1245 项全绿**（无增减：断言改写在既有测试内），`dotnet build -c Release --no-incremental` 0 警告，`dotnet build src/godot/Siege.Godot.csproj` 0 警告，两个 change 的 `openspec validate --strict` 通过。未提交。

### 改动文件

| 文件 | 内容 |
|---|---|
| `src/Siege.Core/Board/Maps/FrontierMapV1.cs` | 六个营帐 `T`（E26 U25 H15 T15 F7 W6）改成普通平台格；解析器不再认 `T`（方块内外出现都当场抛出）；注释"据点 16" → "据点 10" |
| `maps/siege-frontier-v1.json` | `map --map siege-frontier-v1` 重新导出。据点 16 → 10（篝火 6 + 石碑 4）；可落子 411、平台格数、信物 16、五类距离均不变 |
| `src/Siege.Core/Board/MapValidator.cs` | 声明表边疆档 4 人据点区间 `12, 22` → `8, 16`（只动表里这一行与表头注释；规格档分支未增，`规格档分流守门Tests` 全绿）。标准档、v4、`SiteTier` 枚举未动 |
| `FrontierMapLayout.Sites.cs` | `PlaceTentAndRelics` → `PlacePlatformRelics`：删营帐与"离缓坡最远"的平台内最短路，只放 1–2 个信物（彼此 ≥ 2 步）。营帐原本不消费随机数，随机序列的消费次序不变 |
| `FrontierMapGenerator.cs` | `CheckLayout`：营帐数必须为 0 且任何据点不得在出生区内；篝火 = 平台数、石碑 4、公共信物 7 不变 |
| `src/Siege.Sim/Program.cs` | `map` 子命令：某档据点一个都没有时不再打印空的"营帐（T）"行（v4 输出不变） |
| 规格 / 设计 | `frontier-map/specs/map-definition`（预算表 12–22 → 8–16；基准地图 bullet ×2；Scenario「平台留白」「资源布点」「边疆档按自己的区间校验」算例据点 16 → 10）；`map-generator/specs/map-generation`（布局规则第一条、据点条目、Scenario「资源布点」）；`map-generator/design.md`（Context 的预算、D1 第 6 步、裁决 18）；`frontier-map/design.md` 预算表；两份 `tasks.md`。HANDOFF.md、`.trellis/spec/` 里没有边疆图营帐 / 据点 16 / 12–22 的引用（只有 v4 的，不动） |

### 既有测试更新（逐条，均因需求变更）

1. `FrontierFixtures.Map()`（边疆档小图夹具，对应规格算例）：去掉 6 个营帐，据点 16 → **10**（6 篝火 + 4 石碑）。
2. `人数适配预算Tests.边疆档按自己的区间校验`：`Sites.Count` 16 → **10**（算例同步）。
3. `人数适配预算Tests.边疆档据点数区间端点`：`11 拒 / 12 过 / 22 过 / 23 拒`、区间文本 `12–22` → **`7 拒 / 8 过 / 16 过 / 17 拒`、`8–16`**；增减据点的基数 16 → 10（跟夹具）。
4. `出生区编号显示Tests.六区图的插旗棋盘快照`：夹具去营帐的连带——第 14 行 L14 的 `T` → `6`；每区显示区号的格数 23 → **24**（25 − 台内信物 1）。
5. `边疆档基准地图Tests.平台留白_平台内无障碍格`：每平台"恰 1 个营帐" → **"方块内没有任何据点"**；信物 1–2 个不变。
6. `边疆档基准地图Tests.资源布点`：据点 16 → **10**；"营帐每平台恰 1 个、只被本平台覆盖"一段 → **"没有营帐、任何据点都不在平台内"**；篝火 6 / 石碑 4 / 信物 16 的断言不变。
7. `生成图布局规则Tests.资源布点`：`(营帐, 篝火, 石碑) = (N, N, 4)` 且每平台恰 1 营帐 → **`(0, N, 4)` 且任何据点不在出生区内**。
8. `生成确定性Tests` 的 `gen:12345` 黄金值：SHA-256 `5DABB9BF…4FCF3986` → **`CF4009DE…3ADBE5D6`**；尝试序号 0、可落子 370 不变（营帐没了，平台内信物的候选格集合变了）。
9. `地图子命令Tests`：`据点 16（营帐 6，篝火 6，石碑 4）` → **`据点 10（营帐 0，篝火 6，石碑 4）`**；第 15 行两处 `2T` → `25` / `26`。

其余既有测试未改期望。

### 变异验证（脚本二进制读写、finally 还原；每条跑 6 个相关测试类 118 例；末尾全量复绿 1245）

| 编号 | 变异 | 结果 |
|---|---|---|
| M-A21 | 声明表边疆档据点区间改回 `12, 22` | 红（57 例）：`边疆档据点数区间端点`、`边疆档按自己的区间校验`、手工图校验、生成图全体（据点 9–12 低于 12 → 逐次被校验拒绝） |
| M-B16r | 字符画 H15 改回 `T` | 红（20 例）：未知字符当场抛出，`边疆档基准地图Tests` 全类 + 磁盘一致 + `地图子命令` |
| M-B16 | 解析器恢复 `T` + H15 改回 `T` | 红 5：`平台留白_平台内无障碍格`、`资源布点`、磁盘一致 ×2、`地图子命令` |
| MG-22a | 生成器往 1 号平台角上放营帐（自检保留） | 红（48 例）：`CheckLayout` 逐次拒绝 → 耗尽 |
| MG-22 | 同上 + `CheckLayout` 去掉无营帐判据 | 红：`资源布点`（4 个平台数）+ gen:12345 黄金值——新断言本身生效 |

### 牵连检查

- **AI**：`src/Siege.Core/Ai` 里没有任何针对营帐（`SiteTier.Tent`）的专门处理，据点一律按档位分值走；无营帐不涉及任何分支。
- **分析报告 / 遥测**：`Siege.Sim run --map siege-frontier-v1 --seed 1 --count 1 --max-rounds 6 --out sim-out/noTent-smoke` 正常终局（退出码 0，24 小回合 / 6 大回合）；`analyze` 退出码 0，§10 据点一节营帐行输出"营帐（0 个·局）：被控制 无样本（0/0）……"，主人口径两行仍是"无样本……推不出"，篝火 / 石碑正常出数；`replay` 135 行逐字节一致。
- **图形版**：`BoardView` 的营帐模型 / 色带分支保留（v4 仍用）。

### 生成器种子 1–50 统计

与「修订：平台留白」的表**完全相同**（尝试序号分布、可落子格 316–396 / 317–410 / 333–411 / 351–408；只有耗时不同：中位数 25 / 13 / 13 / 17 ms）——营帐不占可落子格、不消费随机数，布局不变，只是平台内信物的落点变了。生成图据点数 = 平台数 + 4（9–12），在新区间 8–16 内。速览三份文件已重出。

### Godot 自检与截图

`--headless -- --auto-demo --pick-check`：v4（105/105）、`--map=siege-frontier-v1`（411/411）、`--map=gen:12345`（370/370）、`--map=gen:987654321:p8`（367/367）——四条退出码均为 0。截图已重截（未读入上下文）：`sim-out/mapgen-shots/frontier-clean-overview.png`、`sim-out/mapgen-shots/gen-12345-clean-overview.png`。

### 偏离与待裁决

1. **夹具 `FrontierFixtures.Map()` 一并去了营帐**（让夹具与规格算例"据点 10"一致），连带改了 `出生区编号显示Tests` 的一条快照（上面第 4 条）。另一种做法是夹具保留营帐（16 个据点在新区间上界内仍合法）、只改算例断言；选了前者，因为"边疆图平台内无据点"已是规格口径。
2. **校验器没有新增"边疆档平台内不得有据点"的规则**：任务只要求改据点区间；"平台内无据点"由手工图解析器、生成器 `CheckLayout` 与两侧测试保证。设计师手写的边疆档 JSON 若在平台内放营帐，校验器仍放行——要不要升成校验规则待定。
3. `map` 子命令的汇总行仍打印"营帐 0"（只去掉了空的坐标行），保持各图输出格式一致。
4. 上一节的待裁决 1（超限作废约 1%）、2（边长组合变少）、4（候选格上限未在新图上重测——现在据点也少了 6 个）、5（旧日志回放报"地图不一致"）仍然有效；3（营帐落角）作废。
