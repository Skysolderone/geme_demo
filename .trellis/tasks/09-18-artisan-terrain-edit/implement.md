...
每批次提子 0.63 / 0.67 / 0.60 / 0.52 / 0.55 / 0.51。40 与 45 档整局无提子 78 / 94 局，超出 §16「整局无提子 ≤ 5%」，出局。推荐 Safety 35。

### 拍板与落地（裁决 T-13、T-14）

负责人 2026-09-18 选 Safety 35、匠人权重 5；烧林保留为冷门动作、地图不改。`EvaluationWeights.Default.Safety` 27→35，校准注释与 `默认评价权重的校准Tests` 同步（期望 27→35、数据路径断言改 `sim-out/artisan-w5-s`）。

改默认值时暴露一处脆弱断言：`地形改造日志与分析Tests.回放日志改造可重建终局地形并与对局逐项一致` 里「样本中确实有致提子的改造」依赖 AI 实际走法，Safety 一改即红。已把该测试的 AI 权重写死为 Safety 27，不再跟随默认值——校准与日志保真度分离。变异 M-D1：默认 Safety 改 30 → 红 3，二进制还原一致。

### 确认跑 200 局（`sim-out/artisan-final/`，`--difficulty Standard --artisan-weight 5`，不带配置文件）

| 指标 | artisan-w5-s35 | artisan-final |
|---|---|---|
| 领先者胜率A | 24.0% | 24.0% |
