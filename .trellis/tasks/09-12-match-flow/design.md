# 09-12-match-flow — 技术设计

## 权威来源

本任务的架构决策、接口契约、风险取舍与已确认裁决，全部以 **`openspec/changes/add-match-flow/design.md`** 为准。实现前 MUST 完整阅读该文件，重点是：
...
**代码位置**：`src/Siege.Core/Match` —— 对局状态机、阶段机、先手值、出局与终局
...
- 本任务尚无线上形态，回滚即回退提交。
- 若本任务修改了已被下游任务依赖的契约，MUST 同步更新 `openspec/changes/add-match-flow/design.md` 的接口契约表，并在受影响的子任务 `prd.md` 中记录。
