...
/// <summary>
/// 正式结算顺序（设计文档 §6.3）的<b>唯一</b>实现。下游 change 只作为被驱动的步骤，MUST NOT 各自"就近"实现结算。
/// </summary>
/// <remarks>
...
public sealed class SettlementDriver
{
    ...
    public SettlementOutcome Confirm(BatchContext context, IReadOnlyList<Placement> placements)
    {
        ...
        var record = new CaptureRecord(sequence, context.Player, ordered, captures);
