...
/// </list>
/// <para><b>大小即取舍</b>（裁决 2）：两个 5×5 出门三格就是广场（到中央入口 4 步，石碑就在缓坡口），但只有 24 格腾挪、且朝广场的缓坡开了 3 格宽；
...
/// <para><b>据点 16。</b>营帐 6：每个平台腹地 1 个。篝火 6：四个大 / 中平台通向河岸的缓坡口各 1（<c>L21 P23 L7 P8</c>），
...
public static class FrontierMapV1
{
    ...
    /// <list type="bullet">
    /// <item><c>1</c>–<c>6</c> 平台格（h=2，数字是平台编号）；<c>T</c> 平台内的营帐；<c>r</c> 平台内的信物格——二者所属平台由 <see cref="Platforms"/> 的外接方块决定。</item>
    ...
    [
       //A...E....K....P....U....Z   列标尺（每 5 列一个字母；完整的列字母表只在 Coord 里，见 coordinates.md「映射唯一」）
        ...
        "111111111###~###~~~~~~~~~", // 29
        "1r111##11###~###333333#~~", // 28
        "11#111111###~###3r33333~~", // 27
        "11#1T11#1###~###33##333~~", // 26
        "111111##1###~###333T333~~", // 25
        "11111111####~###3333333~~", // 24
        "1##111r11###~.C,33##3r3~~", // 23
        "111111111,.o=..,33333#3~~", // 22
        "1111#1111,C.~...F.#,,##~~", // 21
        "~####,,.....~........o#~~", // 20
        "~###C.......=o.########~~", // 19
        "~####,,###..~..########~~", // 18
    ...
    /// <summary>平台的外接方块：编号、西南角、边长。字符画里方块内只许出现本平台的数字、<c>T</c>、<c>r</c> 与岩石，方块外不许出现平台格。</summary>
    ...
    /// <summary>构建边疆档验证地图。字符画有任何笔误（行宽、未知字符、平台格越出方块、缺中央入口）都当场抛出，不产出半张图。</summary>
    public static MapData Create()
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int x = 0; x < Columns; x++)
            {
                ...
                bool platformGlyph = ch is (>= '1' and <= '6') or 'T' or 'r';
                if (platformGlyph != platform.HasValue && ch != '#')
                {
